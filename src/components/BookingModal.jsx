...
export default function BookingModal({ ticket }) {
  ...
  const handleConfirm = () => {
    // Here called the API
    alert(
      `Successfully booked ${quantity} ticket(s) for ${ticket.price * quantity}`,
    );
    setIsOpen(false);
  };
  ...
  return (
    <div>
      <button
        ...
        className="bg-emerald-600 dark:bg-emerald-500 hover:bg-emerald-500 dark:hover:bg-emerald-400 text-white font-black px-6 py-3.5 rounded-xl transition-all shadow-md active:scale-95 text-sm"
      >
        Book Now
      </button>
